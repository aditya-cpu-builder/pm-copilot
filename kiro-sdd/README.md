...
To install the Gemini Code Template, copy the agents and commands to your Gemini configuration directory:
...
# Install agents
cp -rf agents/ ~/.gemini/agents/

# Install KC commands  
cp -rf commands/kc/ ~/.gemini/commands/

# Install hooks (optional)
cp -rf hooks/ ~/.gemini/hooks/
...
| Hook | Event | Description |
| :--- | :--- | :--- |
| Sound Notifications | `Notification` & `Stop` | **Audio Feedback**: Plays system sounds (Funk.aiff) when Gemini requires input or completes tasks, helping developers stay aware of status changes. |
    ...
    "PreToolUse": [
      {
        "matcher": "Bash",
        "hooks": [
          { "type": "command", "command": "~/.gemini/hooks/deny_check.sh" },
          { "type": "command", "command": "~/.gemini/hooks/check_ai_commit.sh" }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          { "type": "command", "command": "~/.gemini/hooks/formatter.sh" }
        ]
      }
    ]
  }
